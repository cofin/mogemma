...
def test_convert_quantize_int8() -> None:
    ...
    np.testing.assert_allclose(q_scale, [expected_scale], rtol=1e-5)  # type: ignore[arg-type]

    # Reconstruct and check error is minimal
    reconstructed = q_weight.astype(np.float32) * q_scale  # type: ignore[operator]
    np.testing.assert_allclose(reconstructed, w_q, atol=expected_scale * 0.5)  # type: ignore[arg-type]
