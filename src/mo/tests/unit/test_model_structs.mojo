...
def test_kv_cache_hybrid_layout() raises:
    ...
    var layer_head_dims_ptr = alloc[Int64](2)
    layer_head_dims_ptr.store(0, 32)
    layer_head_dims_ptr.store(1, 64)
    var layer_kv_heads_ptr = alloc[Int64](2)
    layer_kv_heads_ptr.store(0, 4)
    layer_kv_heads_ptr.store(1, 1)

    var cache = KVCache(2, 4, 32, 512, 4096, layer_types_ptr, layer_head_dims_ptr, layer_kv_heads_ptr)
    ...
    assert_equal(cache.get_layer_head_dim(0), 32)
    assert_equal(cache.get_layer_head_dim(1), 64)
    assert_equal(cache.get_layer_kv_stride(0), 4 * 32)
    assert_equal(cache.get_layer_kv_stride(1), 1 * 64)
    assert_equal(cache.total_elements(), 512 * 4 * 32 + 4096 * 1 * 64)
    layer_types_ptr.free()
    layer_head_dims_ptr.free()
    layer_kv_heads_ptr.free()
    print("test_kv_cache_hybrid_layout passed")


def test_rope_tables_construction() raises:
    var rope = RoPETables(32, 32, 0.5, 512, 4096)
    assert_equal(rope.head_dim, 32)
    assert_equal(rope.rotary_dim, 16)
    assert_equal(rope.sliding_head_dim, 32)
    assert_equal(rope.full_head_dim, 32)
    assert_equal(rope.full_rotary_dim, 16)
