...
fn test_forward_laurel_gpu() raises:
    var hidden_size = 4
    var bottleneck_dim = 2
    ...
    forward_laurel_gpu(get_ptr(out), get_ptr(x), weights, hidden_size, bottleneck_dim, get_ptr(scratch))
...
fn test_forward_per_layer_mapping_gpu() raises:
    var hidden_size = 4
    var per_layer_dim = 2
...
fn test_forward_altup_predict_and_correct_gpu() raises:
    var hidden_size = 4
    var num_modalities = 4
...
fn test_forward_nano_layer_gpu() raises:
    var hidden_size = 4
    var intermediate_size = 4
    var num_heads = 2
    var num_kv_heads = 1
    ...
    base_layer_weights.q_proj = TensorInfo(Int(q_proj.unsafe_ptr()), num_heads * head_dim, hidden_size)
    base_layer_weights.k_proj = TensorInfo(Int(k_proj.unsafe_ptr()), num_kv_heads * head_dim, hidden_size)
    base_layer_weights.v_proj = TensorInfo(Int(v_proj.unsafe_ptr()), num_kv_heads * head_dim, hidden_size)
    base_layer_weights.o_proj = TensorInfo(Int(o_proj.unsafe_ptr()), hidden_size, num_heads * head_dim)
    ...
    var pl_norm = alloc_ones(hidden_size)
