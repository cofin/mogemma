...
- **Gemma 4 Default Selection:** `GenerationConfig` / `EmbeddingConfig` default to `google/gemma-4-E2B-it` — the latest smallest multimodal variant (text + image + audio, PLE). Users select the 26B-A4B MoE or 31B dense explicitly when they want more capacity.
